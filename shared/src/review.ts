...
export const AGENT_OUTPUT_SCHEMA_PROMPT =
  `{ "findings": [{ "id": string (e.g. "sec-1"), "title": string (short, max 8-10 words, specific names from code, no hedging), "severity": "critical"|"high"|"medium"|"low" (lowercase only; critical = security/production breakers, high = logic bugs/crashes, medium = edge cases/performance, low = minor — be conservative), "category": "security"|"performance"|"architecture"|"code-quality", "file": string (required — path from diff), "line": number (required — line number from diff), "message": string (1-2 sentences: what is wrong; no preamble; do not repeat title), "suggested_fix": string (1-2 sentences, actionable, start with imperative: Add/Check/Validate/Wrap), "confidence": number (0-1, required, e.g. 0.85), "impact": string (required — 1 sentence, concrete consequence, no filler), "reasoning_trace"?: string }], "summary": string }. IMPORTANT: "id" must be a string, "severity" must be lowercase, "confidence" must be a number not a string. "file" and "line" are required — reference exact paths and lines from the diff. Title, message, impact, and suggested_fix must not repeat the same information.`;
